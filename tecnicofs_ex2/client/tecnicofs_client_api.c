...
int tfs_mount(char const *client_pipe_path, char const *server_pipe_path) {
    ...
    if(send_to_pipe(fserver,command, sizeof(char)*(MAX_PIPE_LEN + 1)) != sizeof(char)*(MAX_PIPE_LEN + 1)) {
...
int tfs_unmount() {
    ...
    if(send_to_pipe(fserver, buffer, sizeof(char) + sizeof(int)) != (sizeof(char) + sizeof(int))) {
...
int tfs_open(char const *name, int flags) {
    ...
    if(send_to_pipe(fserver,command,(MAX_PIPE_LEN + 1) * sizeof(char) + 2 * sizeof(int)) != (MAX_PIPE_LEN + 1) * sizeof(char) + 2 * sizeof(int)) {
...
int tfs_close(int fhandle) {
    ...
    if(send_to_pipe(fserver,buffer, sizeof(char) + 2 * sizeof(int)) != (sizeof(char) + 2 * sizeof(int))) {
...
ssize_t tfs_write(int fhandle, void const *buffer, size_t len) {
    ...
    if(send_to_pipe(fserver,command,(1 + len) * sizeof(char) + 3 * sizeof(int)) != ((1 + len) * sizeof(char) + 3 * sizeof(int))) {
        perror("client in tfs_write: error sending request to server\n");
        return -1;

    }
    ssize_t written_bytes = 0;
    if(receive_from_pipe(fclient,command, sizeof(int)) < 0) { 
        perror("client in tfs_write: error receiving from the server\n");
        return -1;
    }

    written_bytes = (ssize_t) ((int*)command)[0];
    //talvez verificar erro
    free(command);
...
ssize_t tfs_read(int fhandle, void *buffer, size_t len) {
    ...
    void *command = malloc(sizeof(char) + sizeof(int)*3);
    void *last_pos = command;
    ...
    if(send_to_pipe(fserver,command,sizeof(char) + 3 * sizeof(int)) != (sizeof(char) + 3 * sizeof(int))) {
    ...
    if(receive_from_pipe(fclient,answer_from_server,sizeof(int) + sizeof(char)*MAX_FILE_LENGTH) < 0) {
        perror("client in tfs_read: error receiving from the server\n");
        return -1;
    }

    ssize_t read_content = 0;
    read_content = (ssize_t) ((int*)answer_from_server)[0];
    if(read_content <= 0)
        return -1;
    memcpy(buffer, (void*) ((int*)answer_from_server+1) , (size_t) read_content);
...
int tfs_shutdown_after_all_closed() {
    ...
    if(receive_from_pipe(fclient,buffer,sizeof(int)) < 0){
